...
import { findAdmin } from "@deployi/server/services/admin";
import { updateUser } from "@deployi/server/services/user";
	...
	try {
		...
		scheduleJob(LOG_CLEANUP_JOB_NAME, cronExpression, async () => {
			try {
				...
				await execAsync("docker exec deployi-traefik kill -USR1 1");
			} catch (error) {
