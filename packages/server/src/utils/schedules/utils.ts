...
import type { Schedule } from "@deployit/server/db/schema/schedule";
import { createDeploymentSchedule } from "@deployit/server/services/deployment";
import { updateDeploymentStatus } from "@deployit/server/services/deployment";
import { findScheduleById } from "@deployit/server/services/schedule";
...
export const runCommand = async (scheduleId: string) => {
	...
	} else if (scheduleType === "deployit-server") {
		try {
			const writeStream = createWriteStream(deployment.logPath, { flags: "a" });
