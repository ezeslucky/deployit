...
import type { Schedule } from "@deployi/server/db/schema/schedule";
import { createDeploymentSchedule } from "@deployi/server/services/deployment";
import { updateDeploymentStatus } from "@deployi/server/services/deployment";
import { findScheduleById } from "@deployi/server/services/schedule";
...
export const runCommand = async (scheduleId: string) => {
	...
	} else if (scheduleType === "deployi-server") {
		try {
			const writeStream = createWriteStream(deployment.logPath, { flags: "a" });
