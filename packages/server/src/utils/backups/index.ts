...
import { getAllServers } from "@deployi/server/services/server";
...
import { member } from "@deployi/server/db/schema";
...
export const keepLatestNBackups = async (
	...
	try {
		...
		// --include "*.sql.gz" or "*.zip" ensures nothing else other than the deployi backup files are touched by rclone
