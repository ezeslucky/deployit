	...
	try {
		...
		try {
			...
			const { stdout: postgresContainer } = await execAsync(
				`docker ps --filter "name=deployit-postgres" --filter "status=running" -q | head -n 1`,
			);

			if (!postgresContainer) {
				throw new Error("deployit Postgres container not found");
			}
			...
			await execAsync(
				`docker exec ${postgresContainerId} psql -U deployit postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = 'deployit' AND pid <> pg_backend_pid();"`,
			);

			emit("Dropping existing database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U deployit postgres -c "DROP DATABASE IF EXISTS deployit;"`,
			);

			emit("Creating fresh database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U deployit postgres -c "CREATE DATABASE deployit;"`,
			);
			...
			await execAsync(
				`docker exec ${postgresContainerId} pg_restore -v -U deployit -d deployit /tmp/database.sql`,
			);
