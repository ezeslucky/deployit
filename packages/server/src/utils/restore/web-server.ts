...
import { IS_CLOUD, paths } from "@deployi/server/constants";
import type { Destination } from "@deployi/server/services/destination";
...
export const restoreWebServerBackup = async (
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), "deployi-restore-"));
		...
		try {
			...
			if (!postgresContainer) {
				throw new Error("Deployi Postgres container not found");
			}
			...
			await execAsync(
				`docker exec ${postgresContainerId} psql -U deployi postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = 'deployi' AND pid <> pg_backend_pid();"`,
			);

			emit("Dropping existing database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U deployi postgres -c "DROP DATABASE IF EXISTS deployi;"`,
			);

			emit("Creating fresh database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U deployi postgres -c "CREATE DATABASE deployi;"`,
			);
			...
			await execAsync(
				`docker exec ${postgresContainerId} pg_restore -v -U deployi -d deployi /tmp/database.sql`,
			);
