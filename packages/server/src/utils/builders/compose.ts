...
import type { InferResultType } from "@deployi/server/types/with";
...
export const buildCompose = async (compose: ComposeNested, logPath: string) => {
    ...
    Compose Type: ${composeType} ✅`;
		...
		if (compose.isolatedDeployment) {
			await execAsync(
				`docker network connect ${compose.appName} $(docker ps --filter "name=deployi-traefik" -q) >/dev/null 2>&1`,
			).catch(() => {});
		}
...
Compose Type: ${composeType} ✅`;
	...
	set -e
	{
		...
		${compose.isolatedDeployment ? `docker network connect ${compose.appName} $(docker ps --filter "name=deployi-traefik" -q) >/dev/null 2>&1` : ""}
