...
- `Access Key` -> `Access Key (deployit)` = eg. `002s6acf2639910000d000005`
- `Secret Key` -> `Secret Key (deployit)` = eg. `K00+rIsWqPMhmcgqcyOyb9bqby7pbpE`
- `Region` -> `Region (deployit)` = eg. `eu-central-003, us-east-005, us-west-002, us-west-001, us-west-004, etc` it will depend on the region you are using.
- `Endpoint` -> `Endpoint (deployit)` = eg. `https://s3.us-west-002.backblazeb2.com` you will find this endpoint in the Bucket Card at the Home Page.
- `Bucket` -> `Bucket (deployit)` = eg. `deployit-backups` use the name of the bucket you created.
