...
- `Access Key` -> `Access Key (deployit)` = eg. `AK2AV244NFLS5JTUZ554`
- `Secret Key` -> `Secret Key (deployit)` = eg. `I0GWCo9fSGOr7z6Lh+NvHmSsaE+62Vwk2ua2CEwR`
- `Bucket` -> `Bucket (deployit)` = eg. `deployit-backups` use the name of the bucket you created.
- `Region` -> `Region (deployit)` = eg. `us-east-1, us-west-2, etc` it will depend on the region you are using.
- `Endpoint` -> `Endpoint (deployit) (Optional)` = eg. `https://<bucket-name>.s3.<region>.amazonaws.com` you will find this endpoint in the Bucket Card at the Home Page.
