...
deployit offers a UI to connect to any Docker Registry.
...
- **Image Prefix(Optional)**: Useful when using Cluster feature, to tag your images with a prefix eg. `deployit` will convert to `deployit/my-app:latest`.
...
deployit will default to using its own server.