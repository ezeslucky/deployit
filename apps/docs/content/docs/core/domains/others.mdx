---
title: Others
description: This guide will cover how to configure a domain from other providers for your applications in deployit or panel.
---
...
5. Enter the `Host` name, eg. `api` so it will be `api.deployit.com`.
6. Enter the `IPv4 Address` from your server where the application is hosted eg. `1.2.3.4`.
7. Click `Save`.
8. Go to deployit panel and now you can assign either for `Applications` or `Docker Compose`.
9. Go to `Domains` section.
10. Click `Create Domain`.
11. In the `Host` field, enter the domain name eg. `api.deployit.com`.
