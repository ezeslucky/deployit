---
title: Cloudflare
description: This guide will cover how to configure a Cloudflare domain for your applications in deployit or panel.
---
...
6. Enter the `Host` name, eg. `api` so it will be `api.deployit.com`.
7. Enter the `IPv4 Address` from your server where the application is hosted eg. `1.2.3.4`.
8. Click `Save`.
9. Go to deployit panel and now you can assign either for `Applications` or `Docker Compose`.
10. Go to `Domains` section.
11. Click `Create Domain`.
12. In the `Host` field, enter the domain name eg. `api.deployit.com`.
