...
description: Learn how to deploy your application in production in deployit.
---

import { Callout } from 'fumadocs-ui/components/callout';

By default, deployit offer multiple [Builds Types](/docs/core/applications/build-type) to deploy your application, the most common is `nixpacks` and `heroku buildpacks`
...
1. **Use Git Provider in Your Application**:
   - Repository: `https://github.com/deployit/production-example`
   - Branch: `main`
   - Build path: `/`
...
10. Now let's create application in deployit.
...
5. That's it, now every time you push to your repository, your application will trigger a deployment in deployit.

The deployment will trigger only if the `Tag` matches the one specified in deployit.
...
jobs:
  build-and-push-dockerfile-image:
    ...
    steps:
      ...
      - name: Trigger deployit Deployment
        uses: deployit/deployit-action@v1
        run: |
            curl -X 'POST' \
            'https://<your-deployit-domain>/api/trpc/application.deploy' \
...
You can also use this Github Action [Action](https://github.com/marketplace/actions/deployit-deployment) to automate the deployment.

## Healthcheck & Rollbacks

When using deployit you can also configure healthchecks and rollbacks, this will allow you to configure your application to be able to recover from failures.
