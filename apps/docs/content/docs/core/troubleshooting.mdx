...
3. **Let's Encrypt Certificates**: It's crucial to point the domain to your server's IP **before** adding it in deployit. If the domain is added first, the certificate won't be generated, and you may need to recreate the domain or restart Traefik.
...
services:
  app:
    image: deployit/deployit:latest
    ports:
      - 3000:3000
```
...
Then, when creating the domain in deployit, specify the service name and port, like this:
...
```bash
curl -sSL https://deployit.com/install.sh | ADVERTISE_ADDR=your-ip sh
```
