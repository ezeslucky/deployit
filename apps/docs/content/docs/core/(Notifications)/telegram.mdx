...
Telegram notifications are a great way to stay up to date with important events in your deployit panel. You can choose to receive notifications for specific events or all events.
...
3. Set a name for your bot, eg. `deployit_bot` make sure the name ends with `_bot`.
4. Copy the `Bot Token` and paste it in deployit `Telegram` Modal section.
...
8. Copy the `Chat ID` and paste it in deployit `Telegram` Modal section.
