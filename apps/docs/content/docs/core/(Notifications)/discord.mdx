...
Discord notifications are a great way to stay up to date with important events in your deployit panel. You can choose to receive notifications for specific events or all events.
...
4. Set a name for your webhook, eg. `deployit_webhook`.
5. Click on the `Webhook` you've created and click on copy the `Webhook URL`.
6. Go to deployit `Notifications` and select `Discord` as the notification provider.
