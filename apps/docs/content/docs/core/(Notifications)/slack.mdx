...
Slack notifications are a great way to stay up to date with important events in your deployit panel. You can choose to receive notifications for specific events or all events.
...
1. Go to `https://deployit.slack.com/marketplace/A0F7XDUAZ-webhooks-entrantes` and click on `Add To Slack`.
2. Select the channel that you want to send the notifications to.
3. Click on `Add webhook to channel`.
4. Copy the `Webhook URL`.
5. Go to deployit `Notifications` and select `Slack` as the notification provider.
