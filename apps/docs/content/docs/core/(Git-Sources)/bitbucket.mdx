...
deployit offer a way to connect your Bitbucket Repository to your deployit panel, you can use Groups Names or personal accounts.

Go to `Git` and select `Bitbucket` as the source, then you can use the following options:

- **Bitbucket Username**: Set the username that you want to connect to deployit.
...
Follow the steps to connect your Bitbucket account to deployit.

1. Go to `https://bitbucket.org/account/settings/app-passwords/new` .
2. Set Label: eg. `deployit-Bitbucket-App`. you can choose any name that you want.
...
4. Click on `Create`.
5. Copy the `App Password` and paste it in deployit `Bitbucket` Modal section.
...
`deployit-workspace` copy and paste it in Workspace Name, please make sure to use the slugified name, if you use names like deployit Workspace in this field can cause issues.
...
<Callout type='warn'>
deployit doesn't support Bitbucket Automatic deployments on each push you make to your repository.
</Callout>
