...
Multi server allows you to deploy your apps remotely to different servers without needing to build and run them where the deployit UI is installed.

## Requirements

1. To install deployit UI, follow the [installation guide](en/docs/core/get-started/installation).
...
- [Hostinger](https://www.hostinger.com/vps-hosting?ref=deployit) Get 20% off with this [referral link](https://www.hostinger.com/vps-hosting?REFERRALCODE=1SIUMAURICI97).
...
7. Click `Setup Server` to proceed. There are two tabs: SSH Keys and Deployments. This guide explains the easy way, but you can follow the manual process via the deployit UI if you prefer.
...
<Callout>
  You only need to run this setup once. If deployit updates later, check the
...
10. To check which server an app belongs to, you’ll see the server name at the top. If no server is selected, it defaults to `deployit Server`. Click `Deploy` to start building your app on the remote server. You can check the `Logs` tab to see the build process. For this example, we’ll use a test repo:  
    Repo: `https://github.com/deployit/examples.git`  
    Branch: `main`  
    Build Path: `/astro`
