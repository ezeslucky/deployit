...
deployit requires the following 7 components to be properly configured for the multi-server feature:
...
6. **deployit Network Created**: A Docker network for deployit must be created on the remote server.
