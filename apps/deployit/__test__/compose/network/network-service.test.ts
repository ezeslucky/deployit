...
import { addSuffixToServiceNetworks } from "../../../../../packages/server/src/index";
...
test("It shoudn't add suffix to deployit-network in services", () => {
	...
	expect(service?.networks).toContain("deployit-network");
});
...
test("It shoudn't add suffix to deployit-network in services multiples cases", () => {
	...
	expect(redis?.networks).toHaveProperty("deployit-network");
	expect(dbNetworks["deployit-network"]).toBeDefined();
	expect(apiNetworks["deployit-network"]).toBeDefined();
});
