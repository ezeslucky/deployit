...
import { IS_CLOUD, scheduleJob } from "@deployit/server";
import { removeScheduleJob } from "@deployit/server";
import { db } from "@deployit/server/db";
import { deployments } from "@deployit/server/db/schema/deployment";
import {
	createScheduleSchema,
	schedules,
	updateScheduleSchema,
} from "@deployit/server/db/schema/schedule";
import { runCommand } from "@deployit/server/index";
import {
	createSchedule,
	deleteSchedule,
	findScheduleById,
	updateSchedule,
} from "@deployit/server/services/schedule";
	...
	list: protectedProcedure
		...
		.query(async ({ input }) => {
			const where = {
				...
				"deployit-server": eq(schedules.userId, input.id),
			};
