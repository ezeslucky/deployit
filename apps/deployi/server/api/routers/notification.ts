...
export const notificationRouter = createTRPCRouter({
	...
	testTelegramConnection: adminProcedure
		...
		.mutation(async ({ input }) => {
			try {
				await sendTelegramNotification(input, "Hi, From Deployi 👋");
				return true;
			} catch (error) {
				throw new TRPCError({
					code: "BAD_REQUEST",
		...
		.mutation(async ({ input }) => {
			try {
				...
				await sendDiscordNotification(input, {
					...
					description: decorate(">", "Hi, From Deployi 👋"),
					color: 0xf3f7f4,
				});
	...
	receiveNotification: publicProcedure
		.input(
			z.object({
				ServerType: z.enum(["Deployi", "Remote"]).default("Deployi"),
